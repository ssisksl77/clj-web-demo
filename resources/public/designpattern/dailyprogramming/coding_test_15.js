...
﻿Output: null
*/
function fnc(strList, N) {
  if (!strList) {
    return null;
  }
  var _list = strList.split('->');
  if (!_list.length || N > _list.length) {
    return null;
  }

  var idx = _list.length - N;
  var res = "";
  res = _list.filter(function(e,i) {
    if (i !== idx) {
      return true;
    }
  }).join('->');
...
console.log(fnc("1->2->3->4->5", 2));
console.log(fnc('1->2->3',3));
console.log(fnc('1',1));
